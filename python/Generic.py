...
def BinomialEffError(k,N):
    ...
    if N==0: 
        print "Total number of events is zero"
        return 0.0
    if k>N:
        print "Passed events > total events!"
        return 0.0
    #print "Efficiency is:", float(k)*100/N, "+-", (1/N)*np.sqrt(k*(1-(k/N)))*100
    return (1/N)*np.sqrt(k*(1-(k/N)))

def SmallerThan(SignalHisto,BkgHisto,Marker):
    SoverSqrtSpB=[]
    ZHisto=BkgHisto.Clone("Significance_"+Marker)
    EffSHisto=BkgHisto.Clone("Eff_S_"+Marker)
    EffBHisto=BkgHisto.Clone("Eff_B_"+Marker)
    for i in xrange(1,BkgHisto.GetNbinsX()+2):
        B=BkgHisto.Integral(0,i)
        ...
        EffSHisto.SetBinContent(i,EffS)
        EffSHisto.SetBinError(i,BinomialEffError(S,SignalHisto.Integral(0,BkgHisto.GetNbinsX()+2)))
        EffBHisto.SetBinContent(i,EffB)
        EffBHisto.SetBinError(i,BinomialEffError(B,BkgHisto.Integral(0,BkgHisto.GetNbinsX()+2)))
        SoverSqrtSpB.append(Z)

    ZHisto.SetBinContent(0,0.0), ZHisto.SetBinContent(BkgHisto.GetNbinsX()+2,0.0)
    ZHisto.SetBinError(0,0.0), ZHisto.SetBinError(BkgHisto.GetNbinsX()+2,0.0)
    EffSHisto.SetBinContent(0,0.0), EffSHisto.SetBinContent(BkgHisto.GetNbinsX()+2,0.0)
    EffSHisto.SetBinError(0,0.0), EffSHisto.SetBinError(BkgHisto.GetNbinsX()+2,0.0)
    EffBHisto.SetBinContent(0,0.0), EffBHisto.SetBinContent(BkgHisto.GetNbinsX()+2,0.0)
    EffBHisto.SetBinError(0,0.0), EffBHisto.SetBinError(BkgHisto.GetNbinsX()+2,0.0)

    return SoverSqrtSpB, ZHisto, EffSHisto, EffBHisto

def GreaterThan(SignalHisto,BkgHisto,Marker):
    SoverSqrtSpB=[]
    ZHisto=BkgHisto.Clone("Significance_"+Marker)
    EffSHisto=BkgHisto.Clone("Eff_S_"+Marker)
    EffBHisto=BkgHisto.Clone("Eff_B_"+Marker)
    for i in xrange(1,BkgHisto.GetNbinsX()+2):
        B=BkgHisto.Integral(i,BkgHisto.GetNbinsX()+2)
        S=SignalHisto.Integral(i,BkgHisto.GetNbinsX()+2)
        if (S+B)==0.0:
            Z=0.0
        else:
            Z=float(S)/np.sqrt(S+B)
        ...
        ZHisto.SetBinError(i,0.0)
        EffSHisto.SetBinContent(i,EffS)
        EffSHisto.SetBinError(i,BinomialEffError(S,SignalHisto.Integral()))
        EffBHisto.SetBinContent(i,EffB)
        EffBHisto.SetBinError(i,BinomialEffError(B,BkgHisto.Integral()))
        SoverSqrtSpB.append(Z)
