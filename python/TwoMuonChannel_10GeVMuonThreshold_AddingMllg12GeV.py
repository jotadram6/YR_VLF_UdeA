...
#BasePath="/home/jose/YRNtuples/"
BasePath="/home/joser/VLF_YR/"
#Signal
...
#SigChain.Add(BasePath+"OfficialSignals_20183108/VLF_MF_200_MS_190_BR_100_0_PU200.root")
SigChain.Add(BasePath+"OfficialSignals_20183108/VLF_MF_100_MS_80_BR_100_0_PU200.root")
...
DYchain=ROOT.TChain("Delphes")
DY0chain=ROOT.TChain("Delphes")
DY1chain=ROOT.TChain("Delphes")
DY2chain=ROOT.TChain("Delphes")
DY3chain=ROOT.TChain("Delphes")

W0Jetschain.Add(BasePath+"MergedRootFiles_20180622/W0J_total.root")
W1Jetschain.Add(BasePath+"MergedRootFiles_20180622/W1J_total.root")
W2Jetschain.Add(BasePath+"MergedRootFiles_20180622/W2J_total.root")
W3Jetschain.Add(BasePath+"MergedRootFiles_20180622/W3J_total.root")
TTchain.Add(BasePath+"MergedRootFiles_20180622/TT_total.root")
DYchain.Add(BasePath+"MergedRootFiles_20180622/DYJs_total.root")
DY0chain.Add(BasePath+"MergedRootFiles_20180622/DY0J_total.root")
DY1chain.Add(BasePath+"MergedRootFiles_20180622/DY1J_total.root")
DY2chain.Add(BasePath+"MergedRootFiles_20180622/DY2J_total.root")
DY3chain.Add(BasePath+"MergedRootFiles_20180622/DY3J_total.root")
...
print "Number of events for DY LowMass=", DYEvents, " XS=", DYXS, " Weight=", DYW, " Passed events=", float(DYEventsP)/DYEvents
print "Number of events for DY+0J=", DY0Events, " XS=", DY0XS, " Weight=", DY0W, " Passed events=", float(DY0EventsP)/DY0Events
print "Number of events for DY+1J=", DY1Events, " XS=", DY1XS, " Weight=", DY1W, " Passed events=", float(DY1EventsP)/DY1Events
print "Number of events for DY+2J=", DY2Events, " XS=", DY2XS, " Weight=", DY2W, " Passed events=", float(DY2EventsP)/DY2Events
print "Number of events for DY+3J=", DY3Events, " XS=", DY3XS, " Weight=", DY3W, " Passed events=", float(DY3EventsP)/DY3Events
print "--------------------------------------------------------------------------------------------"

#Cuts

BasicSelection="((NTightMuons==2)&&(muon1Charge*muon2Charge<0)&&((TwoMuonSystemMass<60)||(TwoMuonSystemMass>120))&&(TwoMuonSystemMass>12))"
TTbarRejection="(NJets<3)"
#LowPTMuonRejection="((muon1Pt>10)&&(muon2Pt>5))"
LowPTMuonRejection="((muon1Pt>10)&&(muon2Pt>10)&&(jet1Pt>70))"
METCut="(MET>60)"
MuonCleansing="(muon2JetsMuon0p4PtRatio>0.2)"
#DYRejection="((muon1Pt<30)&&(muon2Pt<30))"
DYRejection="((muon2Pt/muon1Pt)>0.2)"
FinalCut="(MTmin<60)"
TriggerOnly=LowPTMuonRejection+"&&"+BasicSelection
TriggerOnlyMC=LowPTMuonRejection+"&&"+BasicSelection+"&&"+MuonCleansing
Cut1=LowPTMuonRejection+"&&"+BasicSelection+"&&"+MuonCleansing+"&&"+TTbarRejection
Cut2=LowPTMuonRejection+"&&"+BasicSelection+"&&"+MuonCleansing+"&&"+TTbarRejection+"&&"+METCut
Cut3=LowPTMuonRejection+"&&"+BasicSelection+"&&"+MuonCleansing+"&&"+TTbarRejection+"&&"+METCut+"&&"+DYRejection
Cut4=LowPTMuonRejection+"&&"+BasicSelection+"&&"+MuonCleansing+"&&"+TTbarRejection+"&&"+METCut+"&&"+DYRejection+"&&"+FinalCut

CutFlow=[TriggerOnly,TriggerOnlyMC,Cut1,Cut2,Cut3,Cut4]
#CutFlow=[Cut4]

VariablesForPlotting=["MET","jet1Pt","muon1Pt","muon2Pt","MTmin","NJets","muon2JetsMuon0p4PtRatio","muon2Pt/muon1Pt"]
OptBinning=["(40,0,400)","(20,0,200)","(80,0,160)","(50,0,100)","(60,0,600)","(8,0,8)","(50,0,1)","(24,0,1)"]
XAxis=["p_{T}^{miss} [GeV]","p_{T}(j_{1}) [GeV]","p_{T}(#mu_{1}) [GeV]","p_{T}(#mu_{2}) [GeV]","M_{T}^{1,2} [GeV]","N(j)","p_{T}(#mu_{2})/#sum_{#Delta R=0.4}jets","p_{T}(#mu_{2})/p_{T}(#mu_{1})"]
#VariablesForPlotting=["NJets"]
#OptBinning=["(30,0,30)"]
#XAxis=["N(j)"]


TIMEStr=time.strftime("%H_%M_%S")
PDFNames="Results_TwoMuonChannel_10GeVMuonThreshold_"+TIMEStr
...
for j in xrange(len(CutFlow)):
    for i in xrange(len(VariablesForPlotting)):
        ...
        DYchain.Draw(VariablesForPlotting[i]+" >> "+HistNameDY+OptBinning[i],CutFlow[j])
        ...
        DYHisto=ROOT.gDirectory.Get(HistNameDY)
        ...
        Sintegral_error=ROOT.Double(0.)
        DYintegral_error=ROOT.Double(0.)
        TTintegral_error=ROOT.Double(0.)
        Wintegral_error=ROOT.Double(0.)
        Sint=SignalHisto.IntegralAndError(0,31,Sintegral_error)
        DYint=FullDY.IntegralAndError(0,31,DYintegral_error)
        TTint=TTHisto.IntegralAndError(0,31,TTintegral_error)
        Wint=FullW.IntegralAndError(0,31,Wintegral_error)
        ...
        if i==5:
            ...
            print "------ %.2f +- %.2f ------ %.2f +- %.2f ------ %.2f +- %.2f ------ %.2f +- %.2f -------" % (Sint,Sintegral_error,DYint,DYintegral_error,TTint,TTintegral_error,Wint,Wintegral_error)
